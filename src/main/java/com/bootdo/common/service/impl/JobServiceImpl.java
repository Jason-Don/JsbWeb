...
public class JobServiceImpl implements JobService {
	...
	public boolean isMsgOn() {
		...
		map.put("jobName", "自动下发任务提醒");
		List<TaskDO> list = taskScheduleJobMapper.list(map);
		if(list.size()>0){
			if("1".equals(list.get(0).getJobStatus())){//开启状态
				return true;
			}else{
				return false;
			}
	...
	public void initSchedule() throws SchedulerException {
		...
		for (TaskDO scheduleJob : jobList) {
			if (!"".equals(scheduleJob.getBeanClass())&&"1".equals(scheduleJob.getJobStatus())) {
