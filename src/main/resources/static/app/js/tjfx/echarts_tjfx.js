...
function getCurrentYear() {
	...
	for (var i = 2; i >= 0; i--) {//近三年
		html += 
			'<div id="' + (currentYear - i) + '" onclick="changeConditions(' + (currentYear - i) + ')"><div>' + (currentYear - i) + '年</div></div>';
	}
...
function initWorkflowEchartsOption() {
	var barChart = echarts.init(document
			.getElementById("echarts-bar-chart-workflow"));
	var dj_baroption = {
		tooltip : {
			trigger : 'axis',
	...
	var pieChart = echarts.init(document
			.getElementById("echarts-pie-chart-workflow"));
	var pieoption = {
		title : {
			text : '按任务来源占比统计',
			subtext : year + '年任务总量：' + totalTotal,
		...
		series : [ {
			...
			data : [ {
				...
				value : tzqtdwjbhzbTotal,
				name : '厅直其它单位交办或转办'
			}, {
				value : sjywdkbmjbTotal,
				name : '上级业务对口部门交办'
			}, {
				value : xjywdkbmsqTotal,
				name : '下级业务对口部门申请'
			}, {
				value : qtTotal,
				name : '其它'
...
var barChart = echarts.init(document
		.getElementById("echarts-bar-chart-overtime"));
var pieChart = echarts.init(document
		.getElementById("echarts-pie-chart-overtime"));
...
/*			var year = 0;
 ...
 $("#"+newYear).addClass("a_click");
 $("#"+newYear).siblings("a").removeClass("a_click");
 year = newYear;
 init();
 }*/
var conditions = [];
var year = 0;
function changeConditions(newYear) {
	var duty_people = "";
	year = newYear;
	conditions = JSON.stringify([ year, duty_people ]);
	$("#" + newYear).addClass("a_click");
	$("#" + newYear).siblings("div").removeClass("a_click");
	year = newYear;
	init();
}
function go_back() {
